...
> 状态日期：2026-07-30。本文记录已经验证的事实和仍待完成的工作；“已完成”仅限所列技术验证范围，不等同于生产上线或端到端业务验收。
...
| WebSocket 实时事件 | 当前 API 读取方式已验证；WebSocket 接口、事件范围、重连、去重和补偿机制待研究 |
...
项目仍处于阶段 1。`agent-wechat` V1 微信入口层已经验证，说明微信消息入口层技术可行。下一步工作集中在 Gateway 设计、Hermes Agent 接入、Skills 系统建设、ERP/S6 接口对接和端到端主链路建设；WebSocket 实时事件仍待研究。在这些环节完成并通过验收前，不对外宣称企业业务自动化已经可用。验证范围详见[agent-wechat V1 入口验证记录](./agent-wechat-validation.md)。
