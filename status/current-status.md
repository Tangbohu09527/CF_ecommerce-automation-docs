...
| --- | --- | --- | --- | --- | --- |
| `CF_agent-gateway` | repository `main`=`b488cf452584e73bc9b752564bf90ea153aa8d18`；docs PR #8=`75287d57…` 仍 OPEN 且 checks green | production Release Git authority 仍为 `b488cf452584e73bc9b752564bf90ea153aa8d18`；immutable image `sha256:b9341ca7df6f952b4d81028c497574c1e22478e4408f98791a28bd9514b215f1` 在线 | Gateway、Poll/Dispatch/Delivery、Controller、队列清空与文本链路通过 | docs-only PR #8 未合并且未重新生产部署；PostgreSQL restore 和业务扩展未完成 | 复核 PR #8 后保持 production authority 分层，继续 restore/容量演练 |
...
| `CF_agent-wechat` | `main`=`92393bc2ae1d89dae9449fc131413979aa2fa2f2`；PR #1 OPEN 且 `mergeable_state=dirty` | 生产容器在线时使用 forced-QR R2 行为 | 登录、auth/chats/messages、文本收发通过 | 生产通过不替代 PR #1 conflict resolution、CI 和 main promotion；现场 Image ID 与 exact source SHA 映射未证明 | 先修 PR #4 基线，再处理 PR #1 dirty conflict 并完成 promotion |
| forced-QR R2 | PR #4=`dc103c8…` OPEN/unstable，测试 Fixture/CI 漂移导致基线门禁失败；PR #5=`ddaa7d4…` OPEN/unstable，文档检查通过但继承 PR #4 失败门禁 | 当前生产契约为 `restart: "no"`、loopback 6174、`cf-internal`、`ENABLE_VNC=0`、Token 只读挂载 | fresh QR、手机扫码、API 放行和 Host reboot 后恢复已验证 | PR #4 未全绿前 PR #5 不得合并；PR #1 最终进入 main 前必须解决 dirty conflict；automatic boot stop gate 未完成 | 修复 PR #4 Fixture/CI 漂移并全绿，再合并 PR #5，最后解决 PR #1 conflict 与 main promotion |
