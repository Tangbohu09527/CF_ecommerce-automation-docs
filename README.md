...
- **已验证：** `CF_agent-gateway` commit `587f59f` 已在 Debian Staging 完成真实微信消息从 Polling / Checkpoint、Message Store、Identity Mapping、Access Control 和 Admission 到 Employee Workspace / AI Thread 的联调。Hermes Runtime、AI 回复回传微信和 Skill 执行链尚未接入；当前不代表生产上线或完整 AI Agent 闭环。
...
| [Gateway Debian Staging 真实微信联调验证记录](./status/gateway-wechat-staging-validation.md) | Gateway 版本、环境、真实入站链路、检查点及权限验证结果 |
...
| `CF_agent-gateway` | **Staging 已验证至 AI Thread：** commit `587f59f` 已完成真实微信入口与权限执行链联调；Context Builder、Task Queue、Hermes Runtime、AI 回复回传微信和 Skill 执行链仍待完成，当前不是生产上线或完整 AI Agent 闭环 |
