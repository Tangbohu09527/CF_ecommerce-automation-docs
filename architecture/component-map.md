...
| `CF_filebrowser-enterprise`（正式企业 File Service） | 企业文件中心的人员入口与受控 API；负责 Permission、API Token、Share、Share capability UI、Persistent Audit、WebDAV 和 OnlyOffice | `f329de2fc6e9296ca949acab4873c30a83d5f5e7` 为 V1 Beta 核心代码冻结候选，相关代码已实现并通过自动化测试；自动化接线与 Debian 验收待完成 | 是唯一正式 File Service；不允许任何调用方绕过 API、权限、capability 或 Persistent Audit |
...
- `CF_filebrowser-enterprise` 同时承载人员入口和受控 API；Permission、API Token、Share 与 capability UI、Persistent Audit、WebDAV 和 OnlyOffice 代码已经实现并通过自动化测试。FileBridge / `filebrowser-agentctl` 仍只按稳定契约提交调用方身份、最小凭证、任务和文件引用。
