...
> 状态日期：2026-07-31。本文给出组件级职责摘要；详细边界见[系统设计](../02_系统设计.md)，实施状态见[当前开发进度](../status/current-progress.md)。
...
当前组件状态可概括为：微信入口层**已验证**；Hermes Adapter、实时事件机制和合并转发解析增强均**待开发**，Hermes Agent 接入、Skills 系统和 ERP/S6 接口也仍**待开发或对接**。入口层验证不代表 Gateway、文件处理或企业业务链路已经贯通。
