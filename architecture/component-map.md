...
> 状态日期：2026-07-29。本文给出组件级职责摘要；详细边界见[系统设计](../02_系统设计.md)，实施状态见[当前开发进度](../status/current-progress.md)。

## 核心组件

| 组件 | 职责 | 当前状态 | 主要边界 |
| --- | --- | --- | --- |
| `agent-wechat` | 微信入口：登录、联系人/聊天/消息读取、消息发送；后续获取图片和文件 | 部署和基础微信读写已验证；附件待技术验证 | 不负责 AI 思考、业务决策、ERP 操作或 Skill 执行 |
