...
> 状态日期：2026-07-30。本文是总体架构视图；组件内部设计、安全边界和数据模型以[系统设计](../02_系统设计.md)为准，当前实施进度以[当前开发进度](../status/current-progress.md)为准。
...
| `agent-wechat` | 微信登录、私聊/群聊文本、文件与 ZIP 消息、引用消息和入口标识读取 | V1 入口已验证；未覆盖格式和实时事件仍待验证或研究 |
...
当前已经完成 `agent-wechat` V1 入口验证，包括微信登录、私聊文本、群聊文本、文件消息、ZIP 文件、引用消息、`sender` 识别和 `chatId` 识别。微信消息入口层技术可行。当前消息读取采用已验证的 API 方式，WebSocket 实时事件仍待研究。
