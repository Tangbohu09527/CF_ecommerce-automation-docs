...
- **文档版本：** V1.0
- **文档状态：** 已确认
- **当前阶段：** 阶段1——`agent-wechat` V1 入口与结构化 mention 已验证，Gateway 微信入口与权限执行链已完成 Debian Staging 真实联调，进入 Identity Management、任务与 Hermes 链路建设
...
| Debian 测试节点 `CF_wechat-lab` | **基础环境、微信入口及 Gateway Staging 入站权限链已验证** | 运行在 Hyper-V 中，已验证 `agent-wechat` 微信入口能力；`CF_agent-gateway` commit `587f59f` 已以 venv 方式验证 Polling / Checkpoint、Message Store、Identity、Access Control、Admission、Employee Workspace 和 AI Thread。Gateway Docker、Context Builder、Task Queue、Hermes Adapter 和其他业务服务仍待实施或验证。 |
...
阶段 1 基础测试环境已经完成：Windows AI 节点 `AItest` 与 Hyper-V Debian 测试节点 `CF_wechat-lab` 已具备固定局域网通信、SSH 管理、Docker/Compose 和受控代理出口。`agent-wechat` V1 微信入口及结构化 mention 已完成限定范围验证；合并转发消息已验证类型、发送人和外层标题识别，但内部聊天记录展开与内部文件自动提取仍待开发。`CF_agent-gateway` commit `587f59f` 已在 Debian Staging 以真实微信消息验证 Gateway Runtime、Polling / Checkpoint、Message Store、Identity Mapping、Access Control、Admission、Employee Workspace 和 AI Thread。Context Builder、Task Queue、Identity Management V1、Gateway Hermes Adapter、Hermes Runtime、AI 回复回传微信和 Skill 执行链仍待实现；当前 `hermes_thread_id` 为空。Gateway Docker、图片、Office、PDF、中文文件名、连续多附件、实时事件机制及合并转发内部解析增强仍未验证或完成。该阶段结论不代表生产上线或完整 AI Agent 闭环。环境配置详见[部署运维](./04_部署运维.md)，进度与证据见[当前开发进度](./status/current-progress.md)、[Gateway Debian Staging 真实微信联调验证记录](./status/gateway-wechat-staging-validation.md)和[agent-wechat V1 入口验证记录](./status/agent-wechat-validation.md)。
