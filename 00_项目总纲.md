...
电商业务横跨微信沟通、文件收集、ERP、平台后台、仓储、物流和报表。本项目建立以 CFserver 为权威控制中心、以 Windows AI 主机为 Agent 执行端的企业自动化系统：员工从原微信会话提交请求，系统先保存消息，再执行身份、权限、上下文和路由控制，由 Hermes 调用获准能力，并将可追踪结果返回原会话。
...
| Agent 执行 | Hermes external runtime | 在 Windows AI 主机执行 Agent 和模型调用；不覆盖 CFserver 权威记录 |
...
- Git 权威：`b488cf452584e73bc9b752564bf90ea153aa8d18`。
...
- forced-QR R2 行为已经生产验证，但实现仍位于开放的 PR #1 / PR #4 栈，尚未进入 `main`。
...
| 环境 | 当前职责 |
| --- | --- |
| CFserver | Debian 权威控制中心；承载 Gateway、PostgreSQL 和 external agent-wechat，保存消息与业务链权威状态 |
