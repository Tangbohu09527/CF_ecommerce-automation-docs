...
> 本设计描述准备采用的实现边界。组件职责与权威来源为**已确定**；`CF_agent-gateway` 已落地工程基础和 Message Store Foundation；`CF_filebrowser-enterprise` 的 V1 Beta 核心服务端能力**已实现**，自动化主线对接**待集成**，最终 Debian 部署**待验证**。其他模块、生产数据库与队列选型、接口字段和部署参数仍为**待实现 / 待确认**。
...
截至 2026-08-01，`agent-wechat` V1 微信入口验证已经完成：微信登录、私聊文本、群聊文本、文件消息、ZIP 文件、引用消息、`sender` 识别、`chatId` 识别和文件获取均已验证。合并转发消息已补充验证类型识别、发送人获取和外层标题获取，但尚不能展开内部聊天记录或自动提取内部文件。`CF_agent-gateway` 已完成工程基础和 Message Store Foundation，包括 Conversation / Message / Attachment、消息写入与查询及 `event_id` 幂等，9 项测试通过。Identity Mapping、Employee Conversation Manager、Access Control、Context Builder、Task Queue、WeChat Adapter、Hermes、AI Provider、Worker Bridge、Skills、ERP/S6 接口、实时事件机制、合并转发解析增强和端到端联调仍未完成。具体边界见[当前开发进度](./status/current-progress.md)和[agent-wechat V1 入口验证记录](./status/agent-wechat-validation.md)。
...
文件处理目标顺序如下；Message Store Foundation 已实现，图中其余步骤仍不代表已经实现：
...
以下是 V1 概念清单，不代表数据库产品或全部字段已经定稿。当前代码只已实现 Conversation / Message / Attachment 基础模型：
