...
| Identity Mapping | 将平台稳定来源账号映射为 Enterprise Identity / 企业身份和 Employee Workspace / 员工工作区 | 根据昵称合并员工、授予任务或 Skill 权限 |
| Employee Conversation Manager | 维护 Employee Workspace / 员工工作区、AI Thread / AI 会话线程及来源绑定 | 为每名员工启动独立 Hermes 进程、扩大权限范围 |
...
- Identity Mapping 先把来源平台稳定账号映射到 Enterprise Identity / 企业身份；展示名称不能用于授权或自动合并员工。
...
- Gateway 生成并保存稳定的 `workspace_id` 和 `ai_thread_id`。Hermes Runtime Thread / Hermes 运行时线程的 `hermes_thread_id` 可为空、可重建、可随 Provider 切换而变化，不是系统权威主键。
...
Hermes 输入应是版本化结构，至少包含 `trace_id`、`task_id`、Enterprise Identity / 企业身份引用、`workspace_id`、`ai_thread_id`、可选 `hermes_thread_id`、来源 Physical Conversation / 物理会话、当前指令、上下文快照引用、附件引用、允许的 Skills、权限约束和确认状态。大文件只传引用和元数据。`hermes_thread_id` 只表示 Hermes Runtime Thread / Hermes 运行时线程绑定，不得替代 `ai_thread_id`。
