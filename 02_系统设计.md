...
> 本设计描述目标实现边界，并单独标记当前运行范围。V1 Staging 微信文本消息已经从 Polling、消息与权限控制、Employee Workspace / AI Thread 串通 Hermes API 和原会话回复；Hermes Client、Dispatch、Response Relay、Runtime Thread Binding 与 self message 防回环已验证。Context Builder、Task Queue、完整 Worker Bridge、Skill、图片 / 附件 / 文件链路和生产部署仍为**待实现 / 待验证**。目标群聊线程键保持 `bot_account_id + group_chat_id + sender_id`；Gateway V1 当前 whole-room thread 行为是已知实现偏差，不构成设计变更。上述结论不代表生产上线或完整企业业务自动化。
...
目标主链路是：接收微信事件和附件，校验来源并形成任务批次，固化上下文快照，排队派发给 Windows Worker，Hermes 调用授权 Skill，结果先回到 Debian 持久化，再由微信接入层发送到原会话。Debian 保持权威状态，Windows 节点可替换、可离线恢复。

当前已运行的 V1 Staging 文本链路范围更窄：

```text
员工微信 -> agent-wechat -> WeChat Polling -> Message Store
-> Identity Resolution -> Permission Admission -> Employee Workspace
-> AIThread -> Hermes Dispatch -> Hermes API -> Response Relay
-> WeChat Outbound Sender -> 微信回复
```

该有限文本链路不经过尚未完成的 Context Builder、Task Queue、完整 Worker Bridge 或 Skill 执行链，不能反向把这些目标组件标记为已实现。

CF Gateway 是本文对消息路由、安全隔离及其与 Debian 控制面衔接边界的架构称呼；[Gateway 架构](./architecture/gateway-architecture.md)、[Message Store](./design/message-store-design.md)、[Access Control](./design/access-control-design.md)、[Task Queue](./design/task-queue-design.md)和[员工工作区与 AI 会话线程设计](./design/employee-workspace-design.md)已形成设计基线。当前已完成 V1 Staging 文本闭环，但不表示 Context Builder、Task Queue、完整 Worker Bridge、Skill、文件处理或生产部署已经完成。完整运行证据集中记录在[Gateway V1 Staging 验证记录](./status/gateway-wechat-staging-validation.md)。
...
V1 Staging 已验证以下真实微信文本运行链路：

`微信 -> agent-wechat -> Polling / Checkpoint -> Message Store -> Identity / Permission Admission -> Employee Workspace / AI Thread -> Hermes Dispatch / API -> Response Relay -> WeChat Outbound Sender -> 微信回复`

当前文本链路已经包含 Hermes API 调用、运行时线程绑定和原微信会话回复。Context Builder、Task Queue、完整 Worker Bridge、Skill 执行链、ERP/S6 接口、图片 / 附件 / 文件处理、实时事件机制、合并转发解析增强和生产部署尚未完成。具体证据见[Gateway V1 Staging 验证记录](./status/gateway-wechat-staging-validation.md)。

截至 2026-08-04，`CF_filebrowser-enterprise` 的 `feat/v1-integration` commit `f329de2fc6e9296ca949acab4873c30a83d5f5e7` 为当前代码基线。V1 Beta 核心代码已实现并通过自动化测试，当前进入代码冻结候选、文档收口和 Debian 部署验收阶段；自动化主线、真实客户端和生产发布仍待接入或验收。该实现状态不改变上述 Gateway、Hermes、Skills 或 ERP/S6 的现有进度。

## 微信消息获取方式

- **当前已验证：** 通过 `agent-wechat` API 读取微信消息及入口元数据。
- **当前已验证：** Gateway V1 Polling Runtime 已在 Debian Staging 以 `bootstrap_mode=latest` 建立首次高水位，并在后续真实消息上串通 Checkpoint 与准入链。
- **当前已验证：** Polling 对 `is_self=true` 的机器人自发消息不进入 sink、准入或 Hermes，并继续推进 Checkpoint，避免回复回环。
- **当前已验证：** 当前机器人显示名脱敏记作 `机器人示例名`；从微信成员列表选择该机器人会得到 `isMentioned=true`，真实 `@` 其他成员或只复制 / 输入 `@机器人示例名` 文字时该字段缺失。
- **固定标准化规则：** `is_mentioned = raw.get("isMentioned") is True`；字段缺失按 `false`。不得根据正文 `@`、当前脱敏示例名 `机器人示例名`、旧脱敏示例名 `机器人旧示例名`、引用消息或上一条 mention 推断或继承。
...
- Staging 已验证未知身份拒绝路径，以及授权文本消息创建 Employee Workspace / AI Thread、绑定 Hermes Runtime Thread 并返回原会话。
...
- Gateway 生成并保存稳定的 `workspace_id` 和 `ai_thread_id`，并以 `enterprise_identity_id` 关联工作区所有者。Hermes Runtime Thread / Hermes 运行时线程的 `hermes_thread_id` 可在恢复期间暂时解绑、可重建、可随 Provider 切换而变化，不是系统权威主键。
- V1 Staging 已建立 `ai_thread_id` 到 `hermes_thread_id` 的运行时绑定；`hermes_thread_id` 仍可重建且不是权威主键。
- 跨线程合并、代办或多人协同属于**后续规划**，必须有显式授权和业务规则。

已知实现偏差：Gateway V1 当前 `thread_keys` 忽略 `sender_id`，同群不同员工可能复用 AI Thread。这和上述已确定群聊线程键不一致。目标设计保持不变；在代码修正并通过同群多员工隔离测试前，不得宣称群聊线程隔离已经验收，也不得把 whole-room 行为写成新设计。
...
文件处理目标顺序如下；文字消息 AI 闭环已完成 Staging 运行验证，但本次没有验证附件进入 Context Builder、Task Queue、Hermes、Skill 或 File Service 的完整处理链，图示文件流程仍未端到端运行：

```mermaid
flowchart TB
    W["微信文件"] --> A["agent-wechat API<br/>文件消息与 ZIP 入口已验证"]
    A --> M["Gateway Message Store<br/>基础模型与写入查询已实现"]
    M --> AC["Access Control<br/>文字准入链已完成 Staging 验证"]
    AC -->|"拒绝"| R["仅保留消息历史"]
    AC -->|"允许"| C["Context Builder / Task Queue<br/>待实现"]
    C --> P["AI Router / Provider<br/>待接入"]
    P --> H["Hermes<br/>文件上下文待接入"]
    H --> T["临时文件<br/>待开发"]
    T --> U["解压<br/>待开发"]
    U --> V["OCR / 视觉<br/>按需求评估"]
    V --> S["Skill 处理<br/>待开发"]
...
以下是 V1 概念清单，不代表数据库产品或全部字段已经定稿。当前代码已实现 Conversation / Message / Attachment、企业身份、员工工作区、AI Thread 与 Hermes Runtime Thread 绑定，以及文本响应 Relay 等基础实体或能力；Task、上下文快照、通用持久化结果投递模型和非文本回传等仍待实现：
...
| 状态 | 当前边界 |
| --- | --- |
| **已确定** | 自动化主线接入时，Gateway、Hermes、Skills 和受控客户端只能按稳定 File Service API、用户权限、最小 API Token、Share effective capability 与 Persistent Audit 访问正式文件。 |
| **代码状态** | V1 Beta 核心代码已实现并通过自动化测试；当前为核心代码冻结候选。 |
| **已实现** | Browse / Preview / Download 三权，Create / Modify / Delete / Replace 权限语义，Upload、覆盖、重命名、移动、删除、Range Download、Preview、Thumbnail、Media、ACL、Archive Create / Extract、路径规范化，以及软链接和特殊文件类型防护。 |
| **已实现** | API Token hash-only、旧 Token / Key V5 migration、mandatory migration fail-closed、明文仅创建响应一次性返回、列表仅返回 TokenPrefix 与安全元数据、创建 / 撤销 / 拒绝 Audit；明文不进入持久化前端状态、日志或 Audit。 |
| **已实现** | Share 管理白名单 DTO、九项 capability 显式布尔字段、`configuredCapabilities` / `effectiveCapabilities`、capability 前端 UI、Browse / Preview 派生、Upload Share 强制 Create、密码三态、legacy Token 清理与 credential-active hash 绑定。 |
| **已实现** | Persistent Audit Store、Audit V3 migration、Pending / Finalize / Recovery、RequestID、request-scoped Recorder、degraded 状态、管理员 Audit Query、过滤、分页、用于防止查询分页状态被篡改的签名 Cursor 校验、日志脱敏和主要业务 Audit Action。 |
| **待集成** | FileBridge / `filebrowser-agentctl`、Gateway、Hermes 与 Skills 对稳定 File Service API 的自动化主线接入。 |
| **待验证** | Debian 与 migration 真实环境、生产挂载、备份恢复、升级回滚、Docker / Compose、Nginx / TLS / systemd、真实 WebDAV / OnlyOffice、V1 Beta Candidate、Git tag、GitHub Release 和正式生产上线。 |
| **未完成** | Audit WORM、外部防篡改 Audit 存档、Audit 管理前端 UI 和完整生产 retention 验收。 |
| **禁止** | 不建设第二套正式 File Service；任何自动化程序都不得直接访问正式存储目录。 |

### Capability 计算

- 用户权限与 API Token capability 取交集；任一侧拒绝都不得执行，Token 不能放大用户权限。
- Share DTO 使用九项显式布尔字段；`configuredCapabilities` 保留配置值及 `false`，`effectiveCapabilities` 由服务端结合用户权限和 Share 规则计算，客户端只能按 `effectiveCapabilities` 执行。
- Browse 与 Preview 是派生能力，不能由客户端独立扩权；Upload Share 必须强制 Create，前端显示 configured 与 effective 的降级原因，普通用户和管理员均不能自行放大权限。
- WebDAV 权限控制与 OnlyOffice 保存权限、安全写入使用同一用户、Token、路径和 Audit 边界。

### Share credential 生命周期

- Share 密码更新采用保持、移除、替换三态；字段缺省或 `null` 均表示保持。POST 成功更新和 PATCH 路径更新都清理 legacy `Link.Token`，legacy Token 不能绕过密码认证。
- Share 密码只保存在运行时内存；切换 Share、退出、加载失败和组件卸载时清理，同一 Share 的正常目录导航可以保留。
- credential 必须与 active hash 精确绑定；空 hash 或不匹配 hash 时不得发送 password、token 或 Cookie，A Share 的 credential 不得发送给 B Share。

### Persistent Audit

- Persistent Audit 是 File Service 的安全事实来源；Event 或 SSE 只能传递通知，不能替代持久化审计。
- 高风险写操作在首个业务副作用前执行 `ReservePending`；reservation 失败返回 503 且业务副作用为零。Finalize 失败保留 Pending，由 Recovery 处理；每个外部请求最多形成一个最终 Audit Event。
- Audit 以 RequestID 和 request-scoped Recorder 关联请求，支持 degraded 状态、管理员查询、过滤、分页和用于防止查询分页状态被篡改的签名 Cursor 校验；Action 覆盖 Token、登录、用户、权限、Share、Browse、Preview、Download、Upload、Modify、Rename、Move、Delete、Archive、WebDAV 和 OnlyOffice。
- 密码、Token、PasswordHash、Header、秘密 URL、请求体和宿主机路径不得进入日志或 Audit 明文。

上述结论是 `f329de2fc6e9296ca949acab4873c30a83d5f5e7` 的代码与自动化测试状态，不代表 Debian、真实 WebDAV / OnlyOffice、自动化主线或生产发布已经验收。实现依据和剩余项见[当前开发进度](./status/current-progress.md)。
