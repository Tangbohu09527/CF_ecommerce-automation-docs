...
> 本设计描述准备采用的实现边界。组件职责与权威来源为**已确定**；`CF_agent-gateway` commit `587f59f` 已在 Debian Staging 完成真实微信入口、Polling / Checkpoint、Message Store、Identity Mapping、Access Control、Admission、Employee Workspace 和 AI Thread 的运行联调。Context Builder、Task Queue、Hermes Runtime、AI 回复回传微信和 Skill 执行链仍为**待实现 / 待接入**；生产数据库与队列选型、生产部署参数及完整端到端链路仍为**待确认 / 待验证**。`CF_filebrowser-enterprise` 的 V1 Beta 核心服务端能力**已实现**，自动化主线对接**待集成**，最终 Debian 部署**待验证**。上述 Staging 结论不代表生产上线或完整 AI Agent 闭环。
...
`CF_agent-gateway` commit `587f59f feat: wire wechat polling runtime` 已在 Debian Staging 验证以下运行链路：

`微信 -> agent-wechat -> Gateway Runtime -> Polling -> Checkpoint -> Message Store -> Identity Mapping -> Access Control -> Admission -> Employee Workspace -> AI Thread`

当前运行链路止于 AI Thread。Context Builder、Task Queue、Gateway Hermes Adapter、Hermes Runtime、Worker Bridge、AI 回复回传微信、Skill 执行链、ERP/S6 接口、实时事件机制、合并转发解析增强和完整端到端联调尚未完成。具体证据见[Gateway Debian Staging 真实微信联调验证记录](./status/gateway-wechat-staging-validation.md)，其余边界见[当前开发进度](./status/current-progress.md)和[agent-wechat V1 入口验证记录](./status/agent-wechat-validation.md)。
...
- Staging 拒绝样本已验证未知身份消息保存成功且不创建 Employee Workspace / AI Thread；授权样本已验证创建 `employee_workspaces`、`ai_threads` 和 `thread_source_bindings`。
...
- 当前授权样本中的 AI Thread 已创建，但 Hermes Runtime 未接入，`hermes_thread_id` 为空；下一阶段为 Gateway Hermes Adapter。
...
文件处理目标顺序如下；文字消息的入口权限执行链已完成 Staging 运行验证，但本次没有验证附件进入 Context Builder、Task Queue、Hermes、Skill 或 File Service 的完整处理链，图示流程仍未端到端运行：
